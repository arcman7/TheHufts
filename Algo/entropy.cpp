...
vector<vector<Quote>> algo_test(vector<Quote>& data_set){
    vector<vector<Quote>> signals;
    vector<Quote> buy; vector<Quote> sell;
...
vector<Quote> slide_time_window( int& index, vector<Quote>& data_set, vector<Quote>& window)
{
  vector<Quote> new_window;
...
void initalize_window(int size, vector<Quote>& window, vector<Quote>& data_set){
  for(int i = 0; i < size; i++){
    window.push_back(data_set[i]);
  }
}

double average(vector<Quote>& window){
  double sum = 0;
...
double entropy(vector<Quote>& window, int& pCount, int& nCount){
  //int pCount, nCount;
  double pU,pN;
  pCount = nCount = 0;
  double entropy;
...
vector<vector<Quote>> entropy_algo(vector<Quote>& data_set){
    vector<vector<Quote>> signals;
    vector<Quote> buy; vector<Quote> sell;
    vector<Quote> window;
    int index = 0;
